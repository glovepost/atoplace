...
3. Route with bundled Freerouting JAR
...
**Atopile Phase 2A - Basic Support (Planned):**
- [ ] AtopileProjectLoader - detect and parse `ato.yaml`
- [ ] Board path resolution from entry point
- [ ] CLI auto-detection of atopile projects

**Atopile Phase 2B - Enhanced Metadata (Planned):**
- [ ] Lock file parser (`ato-lock.yaml`) for component values
- [ ] Module hierarchy parser for grouping hints
...
│   │   ├── freerouting.py        # FreeroutingRunner
│   │   ├── net_classes.py        # NetClassAssigner
│   │   ├── diff_pairs.py         # DiffPairDetector
│   │   └── install.py            # Freerouting JAR installer
...
│   ├── workflow.py         # Combined place-and-route workflow
...
Claude: [Reads board://path/components resource]
        ...
        |------|------------|--------------|----------|
        | U1   | QFN-48     | (50.0, 30.0) | 0°       |
        | C1   | 0402       | (48.2, 32.1) | 90°      |
        | ...  | ...        | ...          | ...      |

User: Rotate U1 by 45 degrees.

Claude: [Calls modify_placement("rotate U1 45 degrees")]
        U1 rotated to 45°. Note: This is a non-standard angle that may
...
│                                                                 │
│  ┌─────────────┐     ┌──────────────┐     ┌─────────────────┐  │
│  │  KiCad PCB  │     │  Freerouting │     │   KiCad PCB     │  │
│  │  (placed)   │────►│    Runner    │────►│   (routed)      │  │
│  └─────────────┘     └──────────────┘     └─────────────────┘  │
│         │                   │                     │             │
│         ▼                   ▼                     ▼             │
