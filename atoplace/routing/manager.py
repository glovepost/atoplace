...
from .astar_router import AStarRouter, NetOrderer, RouterConfig, RoutingResult
from .fanout import FanoutGenerator, FanoutResult, FanoutStrategy
...
class RoutingManager:
    ...
    def _run_fanout_phase(self):
        ...
        for ref in bga_refs:
            result = self.fanout_gen.fanout_component(
                ref, strategy=self.config.fanout_strategy, include_escape=True
            )
    ...
    def _run_critical_phase(self):
        ...
        dp_router = DiffPairRouter(self.obstacle_index, self.dfm, self.viz)
        ...
        for dp in self.config.diff_pairs:
            ...
            # Route the diff pair using dedicated router
            # API expects (pad_p, pad_n) tuples for start and end
            start_pads = (pads_p[0], pads_n[0])
            end_pads = (
                pads_p[-1] if len(pads_p) > 1 else pads_p[0],
                pads_n[-1] if len(pads_n) > 1 else pads_n[0],
            )

            result = dp_router.route_pair(dp, start_pads, end_pads)

            if result.success and result.geometry:
                trace_count = len(result.geometry.traces_p) + len(result.geometry.traces_n)
                logger.info(f"Routed diff pair {dp.name}: {trace_count} traces")
                # Add traces to board
                for trace in result.geometry.traces_p + result.geometry.traces_n:
                    self.board.add_trace(trace)
                # Add vias
                for via in result.geometry.vias_p + result.geometry.vias_n:
                    self.board.add_via(via)
                # Mark nets as routed
                self.config.routed_nets.add(dp.net_p)
                self.config.routed_nets.add(dp.net_n)
            else:
                # Log failure reason and let general router handle these nets
                logger.warning(
                    f"Diff pair {dp.name} routing failed: {result.failure_reason}. "
                    "Will attempt with general router."
                )
    ...
    def _run_general_phase(self):
        ...
        router = AStarRouter(self.obstacle_index, router_config, self.viz)
        ...
        for np in net_pads_list:
            if (
                np.net_name not in self.routed_nets
                and np.net_name in self.nets_to_route
            ):
...
def route_board_managed(
    board: Board, dfm_profile: DFMProfile, config: Optional[RoutingManagerConfig] = None
