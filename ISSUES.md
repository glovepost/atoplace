...
- **Interactive Apply Ignores DFM Spacing**: `cmd_interactive` selects a DFM profile but `apply` runs `ForceDirectedRefiner` with default spacing (0.25/0.5mm), so interactive refinements can violate the chosen DFM rules. File: `atoplace/cli.py`.
- **Interactive Moves Ignore Locks**: `cmd_interactive` uses `ForceDirectedRefiner` with default `lock_placed=False`, so locked components can still move in interactive mode even though the UI suggests constraints are being honored. File: `atoplace/cli.py`.
