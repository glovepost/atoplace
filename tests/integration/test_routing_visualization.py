...
def test_routing_visualizer_export_svg_delta():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        traces = [
            RouteSegment(
                start=(10, 10), end=(50, 10), layer=0, width=0.25, net_id=1
            ),  # F.Cu
            RouteSegment(
                start=(50, 10), end=(50, 50), layer=0, width=0.25, net_id=1
            ),  # F.Cu
            RouteSegment(
                start=(20, 20), end=(60, 20), layer=1, width=0.25, net_id=2
            ),  # B.Cu
        ]
        ...
        assert (
            "deltaFrames" in html_content
        ), "Should contain deltaFrames JavaScript variable"
        ...
        assert (
            "show-top" in html_content or "Top (F.Cu)" in html_content
        ), "Should have F.Cu layer control"
        assert (
            "show-bottom" in html_content or "Bottom (B.Cu)" in html_content
        ), "Should have B.Cu layer control"
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            assert (
                len(frame3["traces"]) == 3
            ), f"Expected 3 traces, got {len(frame3['traces'])}"
            assert (
                len(frame3["vias"]) == 2
            ), f"Expected 2 vias, got {len(frame3['vias'])}"
            ...
            assert (
                len(frame2["astar_explored"]) > 0
            ), "Frame 2 should have A* explored nodes"
            assert (
                len(frame2["astar_frontier"]) > 0
            ), "Frame 2 should have A* frontier nodes"
...
def test_routing_visualizer_empty_frames():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        visualizer = RouteVisualizer(
            board_bounds=(0, 0, 50, 50), output_dir=Path(tmpdir)
        )
...
def test_legacy_export_deprecation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Add a minimal frame
        visualizer.capture_frame(obstacles=[], pads=[], label="Test")
...
def test_frame_capture():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        visualizer = RouteVisualizer(
            board_bounds=(0, 0, 100, 100), output_dir=Path(tmpdir)
        )
