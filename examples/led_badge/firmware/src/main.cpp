...
#define MODE_INTERVAL_TIMER 12
int currentMode = MODE_INTERVAL_TIMER;
...
const unsigned long intervalTimerDuration = 60000UL;
const int intervalTimerRepeats = 15;
unsigned long intervalTimerStartTime = 0;
int intervalTimerCompleted = 0;
bool intervalTimerActive = false;
...
void detectBeat()
{
  ...
  if (currentBassEnergy > averageBassEnergy * beatThreshold &&
      ...
      averageBassEnergy > 10)
  { // Minimum energy threshold to avoid noise
    beatDetected = true;
...
void drawIntervalProgress(float progress)
{
  strip.clear();
  if (progress < 0.0f)
  {
...
void initIntervalTimer()
{
  intervalTimerActive = true;
  intervalTimerCompleted = 0;
  intervalTimerStartTime = millis();
  drawIntervalProgress(0.0f);
}
...
void updateIntervalTimer()
{
  ...
  unsigned long elapsed = now - intervalTimerStartTime;

  if (elapsed >= intervalTimerDuration)
  {
    drawIntervalProgress(1.0f);
    intervalTimerCompleted++;

    if (intervalTimerCompleted >= intervalTimerRepeats)
    {
    ...
    intervalTimerStartTime = now;
    return;
  }

  float progress = (float)elapsed / (float)intervalTimerDuration;
  drawIntervalProgress(progress);
}
...
void setup()
{
  ...
  if (currentMode == MODE_INTERVAL_TIMER)
  {
    initIntervalTimer();
  }
}
...
void loop()
{
  ...
  if ((millis() - lastDebounceTime) > debounceDelay)
  {
    if (reading != buttonState)
    {
      ...
      if (buttonState == LOW)
      {
        currentMode = (currentMode + 1) % 13; // Now we have 13 modes
        strip.clear();
        strip.show();
        ...
        else if (currentMode == MODE_INTERVAL_TIMER)
        {
          Serial.println("Entering Interval Timer mode...");
          initIntervalTimer();
        }
      }
    }
